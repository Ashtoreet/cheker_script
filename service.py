...
class Service:
    ...
    headers = (
        ...
        'Сведения из реестра недобросовестных поставщиков (подрядчиков, '
        'исполнителей) и реестра недобросовестных подрядных организаций',
        ...
        'Система информирования банков о состоянии обработки электронных '
        'документов (311-П, 440-П)',
    ...
    def nalog_svl_do(self, driver):
        ...
        url = self.services[4]
        driver.get(url)
        username = self.input_key(driver, 'svlform_inn', self.inn)
        self.cap_loop(username, driver, 'btn-ok')
        try:
